...
BubbleDetect._floodFill = function(img, w, h, data, relX, relY, log, wantMask = false) {
  ...
  const BRIGHT = 218;
  const isBright = (x, y) => lumAt(x, y) > BRIGHT;
  let seedX = clampInt(Math.round(relX * w), 0, w - 1), seedY = clampInt(Math.round(relY * h), 0, h - 1);
  if (!isBright(seedX, seedY)) { const found = nearestBright(seedX, seedY, w, h, isBright, 26); if (!found) return null; seedX = found.x; seedY = found.y; }
  const maxArea = Math.floor(w * h * 0.22), minArea = Math.floor(w * h * 0.004);
  const visited = new Uint8Array(w * h), component = wantMask ? new Uint8Array(w * h) : null;
  const rowMin = wantMask ? new Int32Array(h).fill(w) : null, rowMax = wantMask ? new Int32Array(h).fill(-1) : null;
  const stackX = new Int32Array(maxArea + 4), stackY = new Int32Array(maxArea + 4); let sp = 0;
  stackX[sp] = seedX; stackY[sp++] = seedY; visited[seedY * w + seedX] = 1;
  let minX = seedX, maxX = seedX, minY = seedY, maxY = seedY, count = 0, leaked = false;
  while (sp > 0) {
    const x = stackX[--sp], y = stackY[sp]; count++;
    if (count > maxArea) { leaked = true; break; }
    minX = Math.min(minX, x); maxX = Math.max(maxX, x); minY = Math.min(minY, y); maxY = Math.max(maxY, y);
    if (wantMask) { component[y*w+x] = 1; rowMin[y] = Math.min(rowMin[y], x); rowMax[y] = Math.max(rowMax[y], x); }
    const nx = [x+1,x-1,x,x], ny = [y,y,y+1,y-1];
    for (let k=0;k<4;k++) { const xx=nx[k], yy=ny[k]; if(xx<0||xx>=w||yy<0||yy>=h) continue; const idx=yy*w+xx; if(visited[idx]||!isBright(xx,yy)) continue; visited[idx]=1; if(sp<stackX.length){stackX[sp]=xx;stackY[sp++]=yy;} }
  }
  if (leaked || count < minArea) return null;
  const padX = (maxX-minX)*0.08+4, padY=(maxY-minY)*0.08+4;
  const x0=Math.max(0,Math.floor(minX-padX)), y0=Math.max(0,Math.floor(minY-padY));
  const x1=Math.min(w,Math.ceil(maxX+padX+1)), y1=Math.min(h,Math.ceil(maxY+padY+1));
  const rect={x:x0/w,y:y0/h,w:(x1-x0)/w,h:(y1-y0)/h};
  if(!wantMask) { if(log) log(`bubble: found ${count}px, bbox=(${minX},${minY})-(${maxX},${maxY}) of ${w}x${h}`); return rect; }
  const sx=img.width/w, sy=img.height/h, cw=x1-x0, ch=y1-y0;
  const crop=document.createElement('canvas'); crop.width=cw; crop.height=ch;
  crop.getContext('2d').drawImage(img,x0*sx,y0*sy,cw*sx,ch*sy,0,0,cw,ch);
  const mask=document.createElement('canvas'); mask.width=cw; mask.height=ch; const md=mask.getContext('2d').createImageData(cw,ch);
  for(let yy=y0;yy<y1;yy++){let a=rowMin[yy],b=rowMax[yy];if(b<a)continue;a=Math.max(x0,a-1);b=Math.min(x1-1,b+1);for(let xx=a;xx<=b;xx++)md.data[((yy-y0)*cw+(xx-x0))*4+3]=255;}
  const mctx=mask.getContext('2d'); mctx.putImageData(md,0,0);
  const out=document.createElement('canvas'); out.width=cw; out.height=ch; const octx=out.getContext('2d'); octx.drawImage(crop,0,0); octx.globalCompositeOperation='destination-in'; octx.drawImage(mask,0,0); octx.globalCompositeOperation='source-over';
  if(log) log(`bubble: extracted masked overlay ${cw}x${ch}`);
  return {...rect,canvas:out};
};
